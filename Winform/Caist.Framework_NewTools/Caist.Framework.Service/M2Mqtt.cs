...
namespace Caist.Framework.Service
{
    public class M2Mqtt
    {
        ...
        public M2Mqtt(string Host, int Port, string ClientId, string UserName, string PassWord)
        {
            this.Host = Host;
            this.Port = Port;
        ...
        public void MqtMessage(string str, string client = "")
        {
            ...
            {
                FrmMian.MyForm.richMQT.Invoke(new Action(() =>
                {
                    string value = string.Format("-{0} - 时间：{1}  内容：{2}\r", client, DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss"), str);
                    FrmMian.MyForm.richMQT.SelectionFont = new Font("宋体", 10, FontStyle.Regular);  //设置SelectionFont属性实现控件中的文本为楷体，大小为12，字样是粗体
                    FrmMian.MyForm.richMQT.SelectionColor = System.Drawing.Color.Red;    //设置SelectionColor属性实现控件中的文本颜色为红色
                    FrmMian.MyForm.richMQT.AppendText(value);
                }));
            }
            else
            {
                if (!string.IsNullOrEmpty(str) && string.IsNullOrEmpty(client))
                {
                    FrmMian.MyForm.richMQT.Invoke(new Action(() =>
                    {
                        ...
                        FrmMian.MyForm.richMQT.SelectionFont = new Font("宋体", 10, FontStyle.Regular);  //设置SelectionFont属性实现控件中的文本为楷体，大小为12，字样是粗体
                        FrmMian.MyForm.richMQT.SelectionColor = System.Drawing.Color.Black;    //设置SelectionColor属性实现控件中的文本颜色为红色
                        FrmMian.MyForm.richMQT.AppendText(value);
                    }));
                }
            }
        ...
        {
            if (!FrmMian.MyForm.MqttStart.Enabled)
            {
                DisposeClose();
            }
    ...
    {
        public PushValues(long code, T v)
        {
            this.code = code;
            this.v = v;
        }
