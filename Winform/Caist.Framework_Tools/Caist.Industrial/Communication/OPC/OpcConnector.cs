...
namespace Communication.OPC
{
    ...
    {
        public static Dictionary<string, IEnumerable<Node>> _opcNodes = new Dictionary<string, IEnumerable<Node>>();
        /// <summary>
        ///     是否正在连接
        /// </summary>
        ...
        ///     Opc客户端
        /// </summary>
        protected static IClientExtend Client;
        ...
        public async Task<object> GetValueByTag(string tag)
        {
            return await Client.ReadAsync<object>(tag);
        }
        ...
        private IEnumerable<Node> subDirectories = null;
        ...
        private async Task<string> SearchTag(string[] tags, char split, int deep, IEnumerable<Node> nodes)
        {
            string key = tags[3].Substring(0, tags[3].LastIndexOf("/"));
            foreach (var node in nodes)
            {
                if (node.Tag != key && deep == 1)
                    continue;
                var currentTag = node.Tag.Substring(node.Tag.LastIndexOf(split) + 1);

                if (_opcNodes.ContainsKey(currentTag))
                    continue;
                if (RegexOn && Regex.IsMatch(currentTag, tags[deep]) || !RegexOn && currentTag == tags[deep])
                {
                    if (nodes.Any(p => tags[3].IndexOf(p.Tag) > -1))
                    {
                        if (currentTag.LastIndexOf("/") > 5)
                        {
                            if (!_opcNodes.ContainsKey(key))
                            {
                                _opcNodes.Add(key, nodes);
                                continue;
                            }
                        }
                    }
        ...
        public async Task<IEnumerable<Node>> Search(string tag)
        {
            var nodes = await Client.ExploreFolderAsync("");
            return await SearchTag(tag, nodes);
        }

        public async Task<IEnumerable<Node>> SearchTag(string tag, IEnumerable<Node> nodes)
        {
            if (nodes.Any(p => p.Tag == tag))
                return nodes;
            foreach (var node in nodes)
            {
                var subDirectories = await Client.ExploreFolderAsync(node.Tag);
                var answerTag = await SearchTag(tag, subDirectories);
                if (answerTag != null) return answerTag;
            }
            return null;
        }
