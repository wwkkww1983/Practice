...
namespace Caist.Framework.Service
{
    ...
    public partial class FrmMian
    {
        ...
        public readonly string mqtTimer = "MQTPushTimer".GetConfigrationStr();
                    ...
                    if (SettingList != null && SettingList.Count() > 0)
                    {
                        ...
                        foreach (var setting in SettingList)
                        {
                            ...
                            if (pointValue != null && pointValue.DictValue != null)
                            {
                                ...
                                {
                                    //没有小数位数的默认位0位
                                    upValue = pointValue.DictValue.ParseToFloat().ToString("f" + setting.DecimalPlaces.ToString() + "").ParseToFloat();
                                }
                        ...
                        if (PushData != null && PushData.values != null && PushData.values.Count() > 0)
                        {
                            ...
                            ushort result = M2Mqtt.Publish(Topic, PushData.ToJson());
                            M2Mqtt.MqtMessage($"向主题：{Topic} 请求发送：{PushData.ToJson()},线程id：{result}");
                            PushCount++; //记录有效推送次数
                        }
                        else
                        {
