...
namespace Caist.Framework.Service.ApplicationManage
{
    public class ViewFunctionService : RepositoryFactory
    {
        ...
        private List<DbParameter> ListFilter(ViewFunctionListParam param, StringBuilder strSql, bool bSystemSettingContent = false)
        {
            ...
            strSql.Append(@" FROM mk_view_function a left join mk_system_setting b on a.system_setting_id = b.id WHERE a.base_is_delete = 0 ");
            ...
            if (param != null)
            {
                ...
                if (!string.IsNullOrEmpty(param.SystemSettingId.ToString()))
                {
                    strSql.Append(" AND a.system_setting_id = @SystemSettingId");
