   ...
   public class ChoucaiMonitorService : RepositoryFactory
    {
        ...
        public async Task<List<ChoucaiMonitorEntity>> GetSecurityInfoList(ChoucaiMonitorParam param)
        {
            ...
            var list = await this.BaseRepository().FindList<ChoucaiMonitorEntity>(strSql.ToString(), filter.ToArray());
            return list.ToList();
        }
        #endregion

        #region 私有方法
        private List<DbParameter> ListFilter(ChoucaiMonitorParam param, StringBuilder strSql)
        {
            strSql.Append(@"SELECT top(1000) a.[dict_Id ] as dictId,a.dict_Value as dictValue, a.create_Time as CreateTime");
            strSql.Append(@" from [dbo].[mk_plc_choucai_values] a  where 1=1");
