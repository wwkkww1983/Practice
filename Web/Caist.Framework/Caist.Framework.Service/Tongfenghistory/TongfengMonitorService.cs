   ...
   public class TongfengMonitorService : RepositoryFactory
    {
        ...
        public async Task<List<TongfengMonitorEntity>> GetSecurityInfoList(TongfengMonitorParam param)
        {
            ...
            var list = await this.BaseRepository().FindList<TongfengMonitorEntity>(strSql.ToString(), filter.ToArray());
            return list.ToList();
        }
        #endregion

        #region 私有方法
        private List<DbParameter> ListFilter(TongfengMonitorParam param, StringBuilder strSql)
        {
            strSql.Append(@"SELECT top(1000) a.[dict_Id ] as dictId,a.dict_Value as dictValue, a.create_Time as CreateTime");
            strSql.Append(@" from [dbo].[mk_plc_tongfeng_values] a where 1=1");
