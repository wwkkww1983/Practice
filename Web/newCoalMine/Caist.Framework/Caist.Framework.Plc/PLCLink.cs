	...
	internal class PLCLink
	{
		...
		public object getValue(string str)
		{
			...
			switch (tag.CheckDataType())
			{
				...
				case PlcValueTypeEnum.TYPE_SHORT:
					result = this.method_15(text, Convert.ToInt32(tag.Address));
					break;
				case PlcValueTypeEnum.TYPE_BYTE:
		...
		public void WriteData(string name, double value)//写数据
		{
			...
			PlcTypeEnum plcTypeEnum = tagGroup.Get_MMtype();
			PlcValueTypeEnum plcValueTypeEnum = tag.CheckDataType();
			if (this.BackTrue(plcTypeEnum, plcValueTypeEnum))
			{
				new WriteData(tagGroup, tag, value);
				this.device.AddWriteData(tagGroup, tag, value);
			}
		}
		private bool BackTrue(PlcTypeEnum e_PLC_MMType_0, PlcValueTypeEnum e_PLC_DATA_TYPE_0)
		{
			return true;
		}
		...
		{
			this.IPSetAdd = FuckProtect.DataFrom(1742);//默认IP
			this.TCPPort = 102;
			this.socket_0 = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
