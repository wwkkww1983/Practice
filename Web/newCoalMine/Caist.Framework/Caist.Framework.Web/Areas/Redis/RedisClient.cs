...
namespace Caist.Framework.Web.Areas.Redis
{
    public class RedisClient
    {
        ...
        /// </summary>
        /// 
        public static string Subscription(VentilatorEnum ventilatorEnum)
        {
            string val = string.Empty;
            try
            {
                var keyvlaue = GetDescriptionByName(ventilatorEnum);
                using (ConnectionMultiplexer redis = ConnectionMultiplexer.Connect("127.0.0.1:6379"))
                {
                    ISubscriber sub = redis.GetSubscriber();
                    sub.Subscribe(keyvlaue, (channel, message) =>
                    {
                        Thread.Sleep(1000);
        ...
        /// <returns></returns>
        public static string GetDescriptionByName(VentilatorEnum enumItemName)
        {
